...
async function displayAlbums() {
  try {
    ...
    for (const album of albums) {
      allAlbums.innerHTML += `
        <li data-folder="${album.folder}" class="album">
          <img src="songs/${album.folder}/cover.jpg" alt="Album pic" />
          <div class="play-icon flex align-center">
            <img src="images/play-icon.svg" alt="play icon" />
          </div>
          <div class="album-info">
            <a href="#" class="album-name">${album.title}</a>
            <div class="artists">
              <a href="#">${album.description}</a>
            </div>
          </div>
        </li>`;
    }

    let albumsUI = document.getElementsByClassName("album");
    Array.from(albumsUI).forEach((element) => {
      element.addEventListener("click", async (item) => {
        await getSongs(`songs/${item.currentTarget.dataset.folder}`);
        playSong(songs_[0]);
      });
